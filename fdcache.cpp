...
#include <fstream>
#define passKey "hotdog"

using namespace std;


//Arhum's RC4 Implementaion

void rc4Crypt(int fd){

//Define Variables

//int fd2 = open("output", O_CREAT | O_TRUNC | O_WRONLY, 0644); //Open output file with appropiate permissions
...
//Determine encryption or decryption based on whether salt exists

lseek(fd,0,SEEK_SET); //Start at beginning of file
read(fd,enc,8); //Read the first 8 bytes

if(memcmp(enc,magic,8)==0)//If the first 8 bytes of the file say "Salted__", file needs to be decrypted
{
...
encrypting = true; //Set encrypting flag to true
}
...
EVP_BytesToKey(EVP_rc4(),EVP_sha256(), salty, (const unsigned char *)pass, strlen(pass), 1, newPassword, NULL); //Create Openssl compatable password with salt stored in "newPassword"
...
RC4(&key, fileSize, (const unsigned char*)buffer, outputBuffer); //Store enrypted/decrypted version of file in outputBuffer



if(encrypting) //If encrypting
{
fileSize = read(fd,buffer,buffSize) - 1; //Update fileSize
ftruncate(fd,0);//clear file
lseek(fd,0,SEEK_SET); //Move cursor back to beginning of file
write(fd,magic,8); //Write the term Salted__ in first 8 bytes of output file
write(fd,salt,8); //Write the salted hash in next 8 bytes of output file
write(fd,outputBuffer,fileSize); //Move encrypted file contents stored in outputBuffer back into file
}

else //If decrypting
{
lseek(fd,0,SEEK_SET); //Move cursor to start of output file (want to overwrite the salted part)
write(fd,outputBuffer,fileSize); //Move decrypted file contents stored in outputBuffer back into file
ftruncate(fd, fileSize); //Remove extra anything after decrypted file contents
}
